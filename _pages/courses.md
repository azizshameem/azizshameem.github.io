---
layout: page
permalink: /courses/
title: courses
description: A tally of some of the academic courses I have undertaken
nav: true
nav_order: 3
---

## Teaching Assistantship
I have been a teaching assistant for the following courses:
- ```DS 203``` Programming for Data Science *(Autumn 2024)*
- ```CS 419``` Introduction to Machine Learning *(Spring 2024)*
- ```MA 106``` Linear Algebra *(Spring 2023)*
- ```MA 205``` Complex Analysis *(Autumn 2022)*

<!-- ## Course Notes
Below are some notes that I have prepared for certain courses:
- ```EE 605``` Error Correcting Codes: *Under [Prof. Nikhil Karamchandani](https://sites.google.com/site/nikhilkaram/)* (notes: [pdf](https://dokania-tanmay.github.io/assets/pdf/ee605.pdf), [sdocx](https://dokania-tanmay.github.io/assets/pdf/ee605.sdocx))
- ```EE 638``` Estimation and Identification: *Under [Prof. Debraj Chakraborty](https://www.ee.iitb.ac.in/wiki/faculty/dc)* (notes: [pdf](https://dokania-tanmay.github.io/assets/pdf/ee638.pdf), [sdocx](https://dokania-tanmay.github.io/assets/pdf/ee638.sdocx))
- ```SC 631``` Games and Information: *Under [Prof. Ankur Kulkarni](https://www.sc.iitb.ac.in/~ankur/)* (notes: [pdf](https://dokania-tanmay.github.io/assets/pdf/sc631.pdf), [sdocx](https://dokania-tanmay.github.io/assets/pdf/sc631.sdocx))

Disclaimer: *The notes have not been verified by the Professors and could have errors. Incase you find any, feel free to reach out to me via email.* -->

## Relevant Coursework

#### Machine Learning and Optimization 
- ```CS772```   Deep Learning for Natural Language Processing *(Spring 2024)*
- ```EE782```   Advanced Topics in Machine Learning *(Autumn 2023)*
- ```CS726```   Advanced Machine Learning *(Spring 2023)*
- ```CS769```   Optimization in Machine Learning *(Spring 2023)*
- ```CS726```   Learning With Graphs *(Autumn 2023)*
- ```EE6106```  Online Learning and Optimisation *(Spring 2023)*
- ```DS303```   Introduction to Machine Learning *(Spring 2022)*
- ```DS303```   Programming for Data Science *(Autumn 2021)*

#### Math and Probability
- ```EE734``` Advanced Probability and Random Processes for Engineers *(Spring 2024)*
- ```EE636``` Matrix Computations *(Spring 2024)*
- ```IE609``` Mathematical Optimization Techniques *(Autumn 2023)*
- ```EE638``` Estimation and Identification *(Autumn 2023)*
- ```EE621``` Markov Chains and Queuing Systems *(Spring 2023)*
- ```EE635``` Applied Linear Algebra *(Autumn 2022)*

#### Others
- ```EE622``` Optimal Control Systems *(Spring 2024)*
- ```EE720``` Number Theory and Cryptography *(Autumn 2023)*
- ```EE613``` NonLinear Dynamical Systems *(Spring 2023)*
- ```EE610``` Image Processing *(Autumn 2022)*
- ```HS301``` Philosophy *(Autumn 2022)*
- ```HS451``` Cognition and Emotion *(Spring 2022)*